...
*/
void OSMDocumentParser::StartElement( const char *name, const char** atts )
{
    ...
    if( strcmp(name,"nd") == 0 )
    {
        if( m_pActWay && atts != NULL )
        {
            ...
            if( strcmp(name,"ref")==0 )
            {
                ...
                m_pActWay->AddNodeRef( nodeRefId );
...
void OSMDocumentParser::EndElement( const char* name )
{
    ...
    {
        m_Relations.push_back( m_pActRelation );
        m_pActRelation = 0;
